...
class Graph {
    ...
    kosaraju() {
        ...
        // Run first DFS for all vertices
        for (let u = 0; u < n; u++) {
            if (!visited[u]) {
                dfs1(u);
            }
...
var longestCycle = function (edges) {
    ...
    for (let u = 0; u < n; u++) {
        const v = edges[u]
        if (v >= 0)
            traditionalEdges.push([u, v])
    }

    g.build(traditionalEdges)

    const { sccId } = g.kosaraju()

    const counts = new Array(n).fill(0);
    for (const root of sccId) {
        counts[root]++;
    }

    let maxSize = -1;
    for (const c of counts) {
        if (c > 1) maxSize = Math.max(maxSize, c);
    }
