...
class Graph {
    ...
    dijkstra(startNode) {
        ...
        const priorityQueue = new MinHeap((a, b) => a[1] - b[1]);
    ...
    prims() {
        ...
        for (const [v, w] of adjList[start]) {
        ...
        while (!pq.isEmpty() && mstEdges.length < n - 1) {
            ...
            for (const [nextV, nextW] of adjList[v]) {
                if (!inMST[nextV]) {
    ...
    kosaraju() {
        ...
        const visit = (u) => {
            ...
            order.push(u); // Store finishing order
        };
        ...
        const sccRoots = new Array(n).fill(null);
        const assign = (u, root) => {
            sccRoots[u] = root;
            for (const [v] of revList[u]) {
                if (sccRoots[v] === null) assign(v, root);
            }
        };

        order.reverse(); // Reverse postorder for SCC
        for (const u of order) {
            if (sccRoots[u] === null) assign(u, u);
        }
     ...
     *
     * todo:
     *    kosaraju: https://leetcode.com/problems/longest-cycle-in-a-graph/description/
     */
}
...
var shortestDistanceAfterQueries = function (n, queries) {
    ...
    for (let query of queries) {
        ...
        console.log(query, distances)
        an.push(distances[n - 1])
    }
